...
export default function BaseStatTable(props: {
  table: CharacterBaseStat[]
  cost?: CharacterAscensionCost[]
}) {
  if (!props.table || props.table.length === 0) {
    return null
  }
  const ascensionStatType = props.table[0].AscensionStatType
  const headers = Object.keys(props.table[0]).filter(
    (key) => key !== "AscensionStatType"
  )
  ...
  return (
    <Table
      header={displayHeaders.map((key, i) => (
        ...
        <tr key={index}>
          ...
          {props.cost && (
            <td className="flex gap-2" style={{ padding: "2px" }}>
              {props.cost
                .find((c) => c.AscensionPhase === row.AscensionPhase)
                ?.materials?.map((material, idx) => {
                  return index % 2 === 1 ? (
                    <Item
                      ...
                      rarity={getCharacterItemCostRarityByIdx(
                        idx,
                        row.AscensionPhase
                      )}
                      scale={0.5}
