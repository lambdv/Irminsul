...
export default function LiteGraphCanvasComponent({ 
  ...
  const handleZoomIn = () => {
    if (canvasInstanceRef.current) {
      ...
      canvas.setZoom(canvas.scale * 1.2, [canvas.canvas.width / 2, canvas.canvas.height / 2]);
    }
  };
  ...
  const handleZoomOut = () => {
    if (canvasInstanceRef.current) {
      const canvas = canvasInstanceRef.current;
      canvas.setZoom(canvas.scale * 0.8, [canvas.canvas.width / 2, canvas.canvas.height / 2]);
    }
  };
  ...
  useEffect(() => {
    ...
    const loadLiteGraph = async () => {
      ...
      LiteGraph.default.registerNodeType("math/add", MathAddNode);
      ...
      LiteGraph.default.registerNodeType("math/multiply", MathMultiplyNode);
      ...
      LiteGraph.default.registerNodeType("basic/number", NumberNode);

      // Create graph instance
      const graph = new LiteGraph.default.LGraph();
      graphRef.current = graph;

      // Create canvas instance
      const canvas = new LiteGraph.default.LGraphCanvas(canvasRef.current, graph);
