...
export default function SignUpCatchAll(props: { message?: string }) {
  const { isSignedIn } = useAuth()
  const router = useRouter()
  const [theme, setTheme] = useState<"dark" | "light">("dark")

  useLayoutEffect(() => {
    // Prevent scrolling on sign-up page - apply to all levels
    // Use useLayoutEffect to apply styles before browser paint
    const html = document.documentElement
    const body = document.body
    const parentContainer = document.querySelector(
      ".focusedPageContentContainer"
    )
    ...
    const originalContainerOverflow = parentContainer
      ? (parentContainer as HTMLElement).style.overflow
      : ""
    const originalContainerHeight = parentContainer
      ? (parentContainer as HTMLElement).style.height
      : ""
    const originalContainerMinHeight = parentContainer
      ? (parentContainer as HTMLElement).style.minHeight
      : ""
  ...
  useEffect(() => {
    document.title = "Sign Up | Irminsul"
  }, [])
