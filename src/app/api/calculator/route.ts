  ...
  try {
    ...
    if (aminus && aminus.StatFactory && aminus.StatTable) {
      try {
        ...
        const characterBaseStats = aminus.StatFactory.get_character_base_stats(characterKey);
        ...
        const weaponBaseStats = aminus.StatFactory.get_weapon_base_stats(weaponKey);
        if (!weaponBaseStats) {
          return NextResponse.json(
        ...
        const totalStats = {
          ...
          totalHP: (unboxedStats.get?.('BaseHP') || 0) * (1 + (unboxedStats.get?.('HPPercent') || 0) / 100) + (unboxedStats.get?.('FlatHP') || 0),
          
          baseATK: unboxedStats.get?.('BaseATK') || 0,
          atkPercent: unboxedStats.get?.('ATKPercent') || 0,
          flatATK: unboxedStats.get?.('FlatATK') || 0,
          totalATK: (unboxedStats.get?.('BaseATK') || 0) * (1 + (unboxedStats.get?.('ATKPercent') || 0) / 100) + (unboxedStats.get?.('FlatATK') || 0),
          
          baseDEF: unboxedStats.get?.('BaseDEF') || 0,
          defPercent: unboxedStats.get?.('DEFPercent') || 0,
          flatDEF: unboxedStats.get?.('FlatDEF') || 0,
          totalDEF: (unboxedStats.get?.('BaseDEF') || 0) * (1 + (unboxedStats.get?.('DEFPercent') || 0) / 100) + (unboxedStats.get?.('FlatDEF') || 0),
          
          em: unboxedStats.get?.('EM') || 0,
          critRate: unboxedStats.get?.('CritRate') || 0,
          critDamage: unboxedStats.get?.('CritDMG') || 0,
          critMultiplier: 1 + ((unboxedStats.get?.('CritRate') || 0) / 100) * ((unboxedStats.get?.('CritDMG') || 0) / 100),
    ...
    if (!character) {
      ...
      character = characters.find(c => c.key === characterKey || c.id === characterKey) || null;
    }
    ...
    if (!weapon) {
      ...
      weapon = weapons.find(w => w.key === weaponKey || w.id === weaponKey) || null;
    }
    ...
    const characterBaseStat = character.base_stats?.find(
    ...
    const weaponBaseStat = weapon.base_stats?.find(
    ...
    const ascensionStatValueStr = characterBaseStat.AscensionStatValue?.replace('%', '') || '0';
    ...
    const weaponSubStatValueStr = (weaponBaseStat.sub_stat_value || '0').replace('%', '');
    ...
    } else if (ascensionStatType === 'ATK%' || ascensionStatTypeLower.includes('atk%')) {
      atkPercent += ascensionStatValue;
    } else if (ascensionStatType === 'DEF%' || ascensionStatTypeLower.includes('def%')) {
      defPercent += ascensionStatValue;
    } else if (ascensionStatType === 'CRIT DMG' || ascensionStatTypeLower.includes('crit dmg') || ascensionStatTypeLower.includes('crit damage')) {
      critDamage += ascensionStatValue;
    } else if (ascensionStatType === 'CRIT Rate' || ascensionStatTypeLower.includes('crit rate')) {
      critRate += ascensionStatValue;
    } else if (ascensionStatType === 'Energy Recharge' || ascensionStatTypeLower.includes('energy recharge')) {
      energyRecharge += ascensionStatValue;
    } else if (ascensionStatType === 'Elemental Mastery' || ascensionStatTypeLower.includes('elemental mastery')) {
      em += ascensionStatValue;
    } else if (ascensionStatTypeLower.includes('dmg bonus') || ascensionStatTypeLower.includes('damage bonus')) {
    ...
    if (weaponSubStatTypeLower.includes('atk') && !weaponSubStatTypeLower.includes('dmg')) {
      if (weaponSubStatTypeLower.includes('%')) {
        atkPercent += weaponSubStatValue;
      } else {
        flatATK += weaponSubStatValue;
      }
    } else if (weaponSubStatTypeLower.includes('hp') && !weaponSubStatTypeLower.includes('dmg')) {
      if (weaponSubStatTypeLower.includes('%')) {
        hpPercent += weaponSubStatValue;
      } else {
        flatHP += weaponSubStatValue;
      }
    } else if (weaponSubStatTypeLower.includes('def') && !weaponSubStatTypeLower.includes('dmg')) {
      if (weaponSubStatTypeLower.includes('%')) {
      ...
      if (weaponSubStatTypeLower.includes('dmg') || weaponSubStatTypeLower.includes('damage')) {
        critDamage += weaponSubStatValue;
      } else {
        critRate += weaponSubStatValue;
      }
    } else if (weaponSubStatTypeLower.includes('energy') || weaponSubStatTypeLower.includes('er')) {
      energyRecharge += weaponSubStatValue;
    } else if (weaponSubStatTypeLower.includes('mastery') || weaponSubStatTypeLower.includes('em')) {
      em += weaponSubStatValue;
    } else if (weaponSubStatTypeLower.includes('dmg bonus') || weaponSubStatTypeLower.includes('damage bonus')) {
    ...
    artifacts?.forEach((artifact: any) => {
      ...
      const value = artifact.value > 0 ? artifact.value : (artifactMainStatValues[statType] || 0);
      
      if (statType === 'HP%' || statType.includes('HP%')) hpPercent += value;
      else if (statType === 'Flat HP' || statType.includes('Flat HP')) flatHP += value;
      else if (statType === 'ATK%' || statType.includes('ATK%')) atkPercent += value;
      else if (statType === 'Flat ATK' || statType.includes('Flat ATK')) flatATK += value;
      else if (statType === 'DEF%' || statType.includes('DEF%')) defPercent += value;
      else if (statType === 'Flat DEF' || statType.includes('Flat DEF')) flatDEF += value;
      else if (statType === 'EM' || statType.includes('Elemental Mastery')) em += value;
      else if (statType === 'CR%' || statType.includes('Crit Rate')) critRate += value;
      else if (statType === 'CD%' || statType.includes('Crit DMG')) critDamage += value;
      else if (statType === 'ER%' || statType.includes('Energy Recharge')) energyRecharge += value;
    ...
    buffs?.forEach((buff: any) => {
      ...
      else if (statType.includes('Elemental DMG%')) elementalDMGBonus += avgValue;
      else if (statType.includes('Physical DMG%')) physicalDMGBonus += avgValue;
      else if (statType.includes('Normal ATK DMG%')) normalATKDMGBonus += avgValue;
      else if (statType.includes('Charge ATK DMG%')) chargeATKDMGBonus += avgValue;
    ...
    // Calculate totals
    const totalATK = (baseATK + weaponBaseATK) * (1 + atkPercent / 100) + flatATK;
    ...
    if (rotation && rotation.length > 0) {
      rotation.forEach((action: any) => {
        ...
        if (multiplier > 0 && instances > 0) {
          ...
          } else if (actionType.includes('charge') || actionType.includes('ca')) {
            dmgBonus += chargeATKDMGBonus / 100;
          } else if (actionType.includes('plunge') || actionType.includes('p')) {
