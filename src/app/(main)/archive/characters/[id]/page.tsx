...
export async function generateStaticParams() {
  ...
  return characters
    .filter((character) => character?.key) // Filter out characters without key
    .map((character) => ({
      id: character.key,
    }))
}
...
export default async function CharacterPage({
  ...
  const color =
    (await getMainColor(getAssetURL("character", data.name, "splash.png"))) ||
    "rgb(73, 73, 73)"
        ...
        >
          ...
          {[data.vision, data.weapon, data.region]
            ...
            .map((item, index) => (
              ...
              >
                <Image
                  src={getCDNURL(`/imgs/icons/${item}.png`)}
                  alt={item}
                  width={100}
                  height={100}
                  style={{
                    ...
                    marginRight: "-2px",
                    filter:
                      theme === "dark" || index == 0 ? "none" : "invert(1)",
                  }}
                />
                {toTitleCase(item)}
              </div>
  ...
  return (
    <RightSidenav>
      ...
      <ul>
        ...
        <li>
          <Link href="#basestats">Base Stats</Link>
        </li>
        <li>
          <Link href="#talents">Talents</Link>
        </li>
        <li>
          <Link href="#passives">Passives</Link>
        </li>
        <li>
  ...
  return (
    <section id="basestats" className={ArchivePageCSS.archiveRecordSection}>
      ...
      <div className="mb-4 overflow-x-auto">
        <BaseStatTable table={data.base_stats} cost={data.ascension_costs} />
      </div>
    </section>
  )
}
  ...
  return (
    <section id="talents" className={ArchivePageCSS.archiveRecordSection}>
      ...
      {data.talents.map((talent, index) => {
        ...
        return <Talent key={index} data={talent} icon={icon} />
      })}
    </section>
  )
}
  ...
  return (
    <section id="passives" className={ArchivePageCSS.archiveRecordSection}>
      ...
      {data.passives.map((passive, index) => {
        ...
        return <Talent key={index} data={passive} icon={icon} />
      })}
    </section>
  )
}
...
function CharacterConstellations({ data }) {
  return (
    <section
      id="constellations"
      className={ArchivePageCSS.archiveRecordSection}
    >
